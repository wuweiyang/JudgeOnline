...
//      const char * CP_J[] = { "javac", "-J-Xms32m", "-J-Xmx256m","-encoding","UTF-8", "Main.java",NULL };
	...
	const char * CP_Y[] = { "python", "-c",
			"import py_compile; import os; py_compile.compile(r'Main.py'); os.system('chmod -R 777 Main.pyc');", NULL };
	...
	if (pid == 0) {
		...
		//@toscl
		execute_cmd("mkdir -p bin usr lib etc/alternatives proc tmp ");
		...
		if (DEBUG)
			printf("status=%d\n", status);
		// @toscl
		execute_cmd("/bin/umount bin usr lib etc/alternatives proc");
...
void run_solution(int & lang, char * work_dir, int & time_lmt, int & usedtime,
	...
	// open the files
	if( lang == 6){
		//freopen("data_test.in", "r", stdin);
		system("python Main.py<data.in > user.out");
	}
	else if( lang == 7){
		system("php Main.php<data.in > user.out");
	}
	else if( lang == 5 ){
		system("./Main.sh<data.in >user.out");	
	}
	else{
...
//      char java_p1[BUFFER_SIZE], java_p2[BUFFER_SIZE];
	...
	case 6: //Python
		// @toscl
		execl("/usr/bin/python", "/usr/bin/python", "Main.py", (char *) NULL);
		break;
	case 7: //php
		//@toscl
